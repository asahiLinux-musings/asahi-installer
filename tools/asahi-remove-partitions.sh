...
# diskutil ap list -plist | plutil -convert -o - - | jq '.Containers | map(select(.PhysicalStores[].DeviceIdentifier | test("^disk0")))|map(select(.Volumes[].DeviceIdentifier == "disk4s1s1"))'
...
function get_all_partitions_for_disk() {
	...
	jq -r --arg devID "${desiredDeviceIdentifier}" "$(jq_get_all_partitions_for_disk)"
}
...
function start_with_macos() {
	...
	bless --mount / --setBoot ||
	  abort_with_error 'Failed to (re)set startup disk' $?
}
...
function asahi_linux_cleaner_privileged() {
	verify_cleaner_prerequisites 1>&2 ||
	  abort_with_error 'Failed to verify cleaner prerequisites' $?
	verify_asahi_partition_assumptions ||
	...
	reboot ||
	  abort_with_error 'Failed to reboot' $?
}

function asahi_linux_cleaner() {
  warn_user
  sudo bash -c "$(declare -f); OUTER_SCRIPT=${OUTER_SCRIPT} asahi_linux_cleaner_privileged"
}

#
# entry point is here
#
asahi_linux_cleaner #2> /dev/null
